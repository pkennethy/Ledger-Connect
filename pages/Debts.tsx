    ...
    const handleSendSMS = () => {
        ...
        Object.keys(categorized).sort().forEach(cat => {
            ...
            periodTxns.forEach(txn => {
                ...
                // If this is a debt with items, hide the total from this line to prevent "sum" redundancy
                const hasItems = txn.type === 'DEBT' && txn.items && txn.items.length > 0;
                
                if (hasItems) {
                    smsBody += `${timeStr} (${displayDesc})\n`;
                } else {
