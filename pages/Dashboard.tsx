...
import { Users, Package, ShoppingCart, TrendingDown, DollarSign, Clock, AlertCircle } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ lang, user }) => {
    ...
    // Admin Stats
    const adminStats = [
        { label: t.total_debt, value: `₱${summary.totalDebt.toLocaleString()}`, icon: <TrendingDown size={24} />, color: 'bg-red-500' },
        { label: t.total_customers, value: MockService.getCustomers().length, icon: <Users size={24} />, color: 'bg-blue-500' },
        { label: t.pending_orders, value: MockService.getOrders().filter(o => o.status === 'PENDING').length, icon: <ShoppingCart size={24} />, color: 'bg-orange-500' },
        { label: t.total_products, value: MockService.getProducts().length, icon: <Package size={24} />, color: 'bg-emerald-500' },
    ];
    ...
    const customerStats = [
        { label: "My Balance Due", value: `₱${myBalance.toLocaleString()}`, icon: <AlertCircle size={24} />, color: 'bg-red-600' },
        { label: "Total Borrowed", value: `₱${totalMyDebt.toLocaleString()}`, icon: <DollarSign size={24} />, color: 'bg-blue-600' },
        { label: "Total Repaid", value: `₱${totalMyPaid.toLocaleString()}`, icon: <TrendingDown size={24} />, color: 'bg-green-600' },
        { label: "Pending Orders", value: MockService.getOrders(user.id).filter(o => o.status === 'PENDING').length, icon: <Clock size={24} />, color: 'bg-orange-500' },
    ];
                         ...
                         {(isAdmin ? MockService.getDebts() : myDebts).slice(0, 5).map(debt => (
                             <div key={debt.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                 <div>
                                     ...
                                     <p className="text-xs text-gray-500">{debt.category} • {debt.items.length} items</p>
                                 </div>
                                 <div className="text-right">
                                     <p className="text-sm font-bold text-red-600">₱{debt.amount}</p>
                                     <p className="text-xs text-gray-400">{new Date(debt.createdAt).toLocaleDateString()}</p>
                                 </div>
                             </div>
                         ))}
